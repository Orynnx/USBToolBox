...
Boot Keyboard + Mass Storage Composite Gadget，并在停止或异常恢复时还原 Android USB。

Core 不提供 NKRO、持续按键状态、Serial、UVC 或 NCM；上层只需声明目标配置并发送一次性
Boot Keyboard chord。
...
- `UsbProfile` 只包含 `keyboard_enabled` 和 `storage_luns`。
